...
# Copyright 2024-2025, Manos Pitsidianakis <manos@pitsidianak.is>
...
ensure_env_var "BASE_REF" 1 || exit $?
ensure_env_var "HEAD_REF" 2 || exit $?
...
echo "Base ref BASE_REF=${BASE_REF}"
echo "Head ref HEAD_REF=${HEAD_REF}"
_BASE_REF=$(get_commit_sha "${BASE_REF}")
_HEAD_REF=$(get_commit_sha "${HEAD_REF}")
echo "Processed base ref _BASE_REF=${_BASE_REF}"
echo "Processed head ref _HEAD_REF=${_HEAD_REF}"

RANGE="${_BASE_REF}..${_HEAD_REF}"
